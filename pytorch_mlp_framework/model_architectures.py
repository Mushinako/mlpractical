...
class ConvolutionalProcessingBlockBatchNormalised(nn.Module):
    def __init__(self, input_shape, num_filters, kernel_size, padding, bias, dilation):
        super(ConvolutionalProcessingBlockBatchNormalised, self).__init__()
    ...
    def forward(self, x):
        out = x
        out = (out - out.mean()) / (torch.std(out) or 1)
...
class ConvolutionalDimensionalityReductionBlockBatchNormalised(nn.Module):
    def __init__(self, input_shape, num_filters, kernel_size, padding, bias, dilation, reduction_factor):
        super(ConvolutionalDimensionalityReductionBlockBatchNormalised, self).__init__()
